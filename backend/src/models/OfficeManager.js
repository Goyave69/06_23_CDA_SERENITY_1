...
async function insertOffice(data) {
  ...
  return connection
    .promise()
    .query(
      "INSERT INTO office (name, doc_name, street_number, street_name, zip_code, phone_number, email, free_parking, disabled, open_hours, specialty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      Object.values(data)
    )
    .then(async ([rows]) => {
...
async function fetchOneOffice(id) {
  return connection
    .promise()
    .query("SELECT * FROM office WHERE idoffice = ?", id)
    .then(async ([rows]) => {
